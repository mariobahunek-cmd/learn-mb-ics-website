...
Every paid invoice, every depreciation run, every incoming payment comes together in the same place: the General Ledger. In SAP S/4HANA it is the heart of financial accounting. It's where all value-relevant business transactions gather, and where the balance sheet and profit-and-loss statement ultimately take shape. Understand that one place, and you have grasped the foundation of external accounting.
...
Perhaps the most important organizational element in finance is the **company code**. It is an independent accounting unit and the smallest organizational element for which a complete, self-contained set of books can be modeled. A typical example is a single legally independent company within a group.
...
The **posting key**, by contrast, sits at line item level, that is, per line in the document. It signals three things to the system: which account type is used (G/L account, customer, vendor, asset, material), whether it is a debit or a credit posting, and which field status applies to the additional entries.
...
An example makes it tangible: an accountant enters a vendor invoice in accounts payable. That invoice appears as an open item on the **vendor account** (the sub-ledger), and at the same time, in the background, the assigned reconciliation account “trade payables” is updated in the General Ledger. There is therefore no difference between the sub-ledger and the General Ledger; they are in sync in real time.

Among others, accounts payable (FI-AP), accounts receivable (FI-AR) and asset accounting (FI-AA) are connected to the General Ledger through reconciliation accounts.
...
- **Mixing up company code and controlling area.** The company code is the reporting entity (FI), the controlling area the organizational unit for Controlling (CO). A controlling area can contain several company codes, all sharing the same operating chart of accounts.
