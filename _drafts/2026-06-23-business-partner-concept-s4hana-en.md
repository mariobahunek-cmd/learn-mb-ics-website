...
As soon as a company runs SAP S/4HANA, one term shows up sooner or later everywhere: the **Business Partner**. Whether in purchasing, sales or accounting — without a Business Partner there is no sales order, no purchase order and no invoice. This article explains, in plain language, what's behind it.

## In short: one master record for all external partners
...
- **Address:** street, postal code, city, country — several addresses are possible.
- **Bank details:** IBAN, BIC, bank name — important for payments.
- **Tax information:** such as the VAT identification number.
- **Communication data:** phone, email and other ways to get in touch.

**Role-specific data** is added the moment you assign a role to the partner:

- **In the customer role:** shipping and payment terms, credit limit.
- **In the supplier role:** order currency, purchasing conditions.
- **In the FI role:** reconciliation account and company-code-related data.
